...
def send_report(config, report_name, status, result_file, recepient_list):
    ...
    <head></head>
    <body>
    <h1>Movie Report: {report_name}</h1>
    <h2>Execution Status:  {status}</h3>
    <p>Dear User, <br /> <br />
        Attached herewith the {report_name} report generated. <br /><br />
        Kindly take the necessary steps.<br /><br />
        --- <br />
        AutoTainment
    </body>
    </html>""".format(report_name=report_name, status=status)
    ...
    zip_file = '{}.zip'.format(report_name)
